  ...
  <script>
    ...
    function ideaVariantPlan(clean, theme, pain, variant=0) {{
      ...
      const plans = {{
        ...
        general: {{
          ...
          scenes:[
            `你总以为再忍一下、再懂事一点，关系就会变好，可最后累的还是你自己`,
            `你不是没有想过改变，而是每次刚想往前走，就开始担心别人会不会失望`,
            `你明明已经不舒服了，却还在替别人找理由，怕自己一开口就显得不懂事`,
            `你表面上是在等一个答案，其实是在等自己终于敢把感受放回第一位`
          ],
          mistakes:['把别人的期待当成自己的选择','把懂事当成安全感','把沉默当成不伤害别人','把拖延当成再等等就会好'],
          methods:[
            ['把今天最不舒服的一件事写下来','删掉“我应该”这三个字','换成“我真实想要的是”'],
            ['先停下自动答应的习惯','给自己十分钟想一想','再决定要不要回应别人'],
            ['把别人的期待列出来','把自己的选择列出来','只选一个今天先还给自己'],
            ['先承认我不想这样了','再说出一个真实需求','最后做一个不会背叛自己的决定']
          ],
          proof:['比如填志愿、换工作、选关系的时候，你最怕的不是选错，而是怕别人说你不听话','以前你会先问别人满不满意，现在你先问自己愿不愿意','你以为拒绝会失去关系，其实真正让关系变累的，是你一直没有出现','今天只做一件小事：把一个“我都可以”，换成“我需要想一下”'],
          interactions:['你现在最不敢违背的是谁的期待？','你有没有也在某个选择里，把自己放到最后？','如果今天只能还给自己一个选择，你会先选哪一个？','你最想从哪一句“我应该”里退出来？'],
    ...
    function topicPackage(topicOrIdea, theme, pain) {{
      ...
      const opening = careerMode
        ? `${{openingLead}}你不是不会做内容，你只是还没让别人一眼看见你到底能解决什么问题。`
        : `${{openingLead}}你不是突然变成这样的，很多时候，是你一路都在先照顾别人的期待，再把自己的感受往后放。`;
      const hotLine = hotTitle
        ? [
            `这两天抖音上「${{hotTitle}}」特别热。${{hotMeta ? `它现在是${{hotMeta}}，` : ''}}能有这么多人讨论，说明它戳中的不是一个孤立的话题，而是很多人正在面对的选择。`,
            careerMode
              ? `同样，做内容也是这样。别人停下来，不是因为你讲了一个热词，而是因为他听见了自己的问题。`
              : `很多人看着热搜，其实想到的是自己：如果人生一直按别人的期待走，那我自己的声音到底放在哪里。`
          ].join('')
        : '';
      const hotBridge = hotTitle
        ? careerMode
          ? `所以别只盯着「${{hotTitle}}」本身，真正能留下人的，是你能不能把一个热闹的话题，落到一个具体的人、一个具体的问题、一个具体的结果。`
          : `如果你也总是在重要选择面前先想别人会怎么评价，那「${{hotTitle}}」热起来的这一刻，其实也在提醒你：别把自己的人生一直交给别人的期待。`
        : '';
      ...
      const viralLine = viralCase
        ? `先给你一个判断：${{plan.angle}}。你别急着照搬别人的爆款句子，真正能让用户停下来的，是你把「${{clean}}」讲到他的真实处境里。`
        : opening;
      const script = careerMode
        ? [
            ...
            hotLine,
            hotBridge,
            `很多人一想做${{themeName}}，第一反应就是多发、追热点、赶紧涨粉。可别人不会因为你很努力就相信你，他只会在几秒钟里判断：这个人懂不懂我，能不能帮我解决眼前的问题。`,
            `真正卡住的地方，往往就是「${{clean}}」。表面看是${{pain}}，背后其实是你的表达还没有让别人看见价值。`,
            `${{plan.scene}}。这种场景里，最有用的不是一句漂亮口号，而是一句能让人停下来的实话。`,
            `很多人会误以为${{plan.mistake}}。可内容做不起来，往往不是没有灵感，也不是不会追热点，而是别人看完以后，还是不知道你帮谁、解决什么问题、为什么值得相信。`,
            `所以这件事要落到三个动作里：${{plan.method[0]}}；${{plan.method[1]}}；${{plan.method[2]}}。当这三件事连起来，内容才不只是热闹，而是能让别人记住你。`,
            `${{plan.proof}}。你不需要把自己讲得很厉害，你只需要让别人看见：你真的理解这个问题，也真的有一个能往前走的办法。`,
        ...
        : [
            ...
            hotLine,
            hotBridge,
            `${{plan.scene}}。你看，真正让人累的不是做一个选择，而是每一次选择之前，都要先想别人会不会满意。`,
            `你可能一直以为这是懂事，其实很多时候，你是在${{plan.mistake}}。所以你一边不舒服，一边还在说服自己：算了，别麻烦，别扫兴，别让别人失望。`,
            `可你越这样活，越容易忘记自己真正想要什么。到最后，别人都觉得你很好说话，只有你自己知道，你已经很久没有认真听过自己的声音。`,
            `从今天开始，${{plan.method[0]}}；${{plan.method[1]}}；${{plan.method[2]}}。这不是让你突然变得很强硬，而是让你慢慢把自己放回自己的生活里。`,
            `${{plan.proof}}。你会发现，一个人真正开始改变，往往不是做了多大的决定，而是终于在一个很小的瞬间，没有再背叛自己。`,
            `最后我想问你一句：${{plan.interaction}}`
          ].filter(Boolean).join('\\n\\n');
