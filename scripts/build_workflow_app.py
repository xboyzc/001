  ...
  <script>
    ...
    function topicPackage(topicOrIdea, theme, pain) {{
      ...
      const opening = careerMode
        ? `${{openingLead}}今天我只跟你聊一个很具体的问题：「${{clean}}」。别一上来就想着涨粉、变现、工具，先把用户为什么愿意相信你这件事说清楚。`
        : `${{openingLead}}今天我就用一个很具体的场景，把「${{clean}}」这件事讲明白。`;
      const hotLine = hotTitle
        ? [
            `这两天抖音上「${{hotTitle}}」特别热。${{hotMeta ? `它现在是${{hotMeta}}。` : ''}}`,
            hotSummary ? `公开信息里最明显的一点是：${{hotSummary}}` : `大家都在主动搜索和讨论它。`,
            `但我更想提醒你的是，热点真正值得看的，不只是新闻本身，而是它背后暴露出来的那个现实问题：${{pain}}。`
          ].join('')
        : '';
      const hotBridge = hotTitle
        ? `把这个热点放到「${{themeName}}」里看，你会发现，用户不是因为一个标题停下来，而是因为他突然意识到：这件事好像和我有关。那我们今天就把「${{hotTitle}}」拉回到「${{clean}}」这个问题上，讲清楚你下一步到底该怎么做。`
        : '';
      ...
      const script = careerMode
        ? [
            ...
            hotLine,
            hotBridge,
            `很多人一想做${{themeName}}，第一反应就是：我得多发作品，我得追热点，我得赶紧涨粉。可用户不会因为你很努力就相信你，他只会在几秒钟里判断一件事：这个人到底懂不懂我，能不能帮我解决眼前的问题。`,
            `所以你真正要先处理的，不是发多少条，而是「${{clean}}」。用户卡住的地方往往不是不知道要努力，而是${{pain}}。`,
            `你可以想象一个很真实的画面：${{plan.scene}}。这时候用户最需要的不是大道理，而是一句话让他感觉：对，说的就是我。`,
            `很多人会误以为${{plan.mistake}}。但内容做不起来，往往不是你没有灵感，也不是你不会追热点，而是用户看完以后，还是不知道你帮谁、解决什么问题、为什么值得相信。`,
            `你今天可以先做三件事。第一，${{plan.method[0]}}。第二，${{plan.method[1]}}。第三，${{plan.method[2]}}。这三件事做完，你的内容就不会只是在热闹里刷存在感，而是在一点点建立别人对你的信任。`,
            `举个更具体的例子：${{plan.proof}}。你不要讲得太大，就把一个真实业务场景说清楚，让用户看到从不信任到愿意继续看你，中间到底发生了什么。`,
            hotTitle ? `所以别怕用热点，但也别只停在热点。「${{hotTitle}}」只是让用户停下来的入口，真正让他留下来的，是你把「${{clean}}」讲得具体、讲得有用、讲得他能马上照着做。` : `最后记住，入口只是入口，信任才是结果。你讲得具体，用户才会留下来；你给出动作，用户才知道下一步怎么跟你走。`,
        ...
        : [
            ...
            hotLine,
            hotBridge,
            `你有没有发现，很多内容听起来都很有道理，可你听完只是点点头，然后生活还是老样子。不是你不想改变，是它没有把你带进一个真实场景里。`,
            `那我们今天就把「${{clean}}」说具体一点。你真正卡住的地方，可能就是：${{plan.scene}}。`,
            `很多人会误以为${{plan.mistake}}，所以一直在原地绕。可真正重要的不是再听一个观点，而是把眼前这一步变成一个能马上做的小动作。`,
            `你现在可以这样开始：先${{plan.method[0]}}；再${{plan.method[1]}}；最后${{plan.method[2]}}。不要等完全准备好，先让自己有一个能动起来的入口。`,
            `举个例子，${{plan.proof}}。你会发现，例子越具体，你越容易判断：这说的就是我，我现在确实可以这样做。`,
